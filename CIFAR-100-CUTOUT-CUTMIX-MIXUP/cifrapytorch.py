...
def main(aug_method):
    ...
    method = aug_method  # baseline, mixup, cutout, cutmix
    ...
    val_loader = torch.utils.data.DataLoader(testset, batch_size=BATCH_SIZE, shuffle=False, pin_memory=True,
    ...
    for epoch in range(epochs):
        ...
        os.chdir(os.path.abspath(os.path.dirname(os.getcwd())))  # 返回文件根目录


if __name__ == '__main__':
    aug_method_list = ['baseline', 'mixup', 'cutout', 'cutmix']
    for aug_method in aug_method_list:
        print('正在训练,加强方法为：' + aug_method + '!')
        time_start = time.time()
        main(aug_method)
        time_end = time.time()
        print('--' * 3 + aug_method + '方法下训练耗时：' + str(round((time_end - time_start) / 60, 2)) + '分钟！' + '--' * 3)
